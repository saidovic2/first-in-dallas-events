    ...
    try:
        ...
        if location_data:
            if isinstance(location_data, dict):
                ...
                city_value = location_data.get('city') or location
                # Normalize city - remove ", TX" suffix for consistency
                event['city'] = city_value.replace(', TX', '') if city_value else None
                event['address'] = location_data.get('street') or location_data.get('address')
            elif isinstance(location_data, str):
                event['venue'] = location_data
                event['city'] = location.replace(', TX', '')
        else:
            event['city'] = location.replace(', TX', '')
