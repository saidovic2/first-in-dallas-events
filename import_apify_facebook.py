"""
Import Facebook events from Apify dataset into CMS
...
def transform_apify_event(apify_event):
    ...
    # Skip online-only events (we want local Dallas events)
    ...
    # Fallback to dateTimeSentence if utcStartDate fails
    if not start_at:
    ...
    if not city:
        ...
        if contextual:
            # Parse "Arlington, TX, United States"
            parts = contextual.split(',')
            if len(parts) >= 1:
                city = parts[0].strip()
    
    # Fallback to Dallas if no city found
    if not city:
        city = 'Dallas'
...
def main():
    ...
    print(f"\n⚠️  About to import {len(events)} Facebook events into your CMS.")
